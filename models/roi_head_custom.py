...
class RoIHeads_custom(nn.Module):
    __annotations__ = {
    ...
    def __init__(self,
        ...
        super(RoIHeads_custom, self).__init__()
        ...
        self.pe_generator = SinePositionalEncoding(num_feats=128,normalize=True)
    def has_mask(self):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                # print(gt_labels_in_image,clamped_matched_idxs_in_image)
    ...
    def forward(self,
        ...
        """
        bs,  h, w = 1,128,128
        mask_pe = torch.zeros((bs, h, w), device=torch.device('cuda'), dtype=torch.bool)
        img_feats_pe = self.pe_generator(mask_pe)
        for key, value in features.items():
            # print(img_feats_pe,features[key])
            features[key] = features[key] +torch.nn.functional.interpolate(img_feats_pe, size=features[key].shape[-2:], mode='bilinear')
